...
import { ChatInputCommandInteraction, GuildEmoji, Message, Role, TextChannel } from 'discord.js'
import CavenBot from '../../structures/CavenBot'

export default class extends Command {
	constructor(client: CavenBot, name='reactroles') {
		super(client, name, {
			aliases: ['rr', 'react'],
	...
	async executeSlash(interaction: ChatInputCommandInteraction) {
		...
		if (interaction.options.getSubcommand() === 'add') {
			...
			guild.channels.fetch(id).then(async (channel: TextChannel) => {
				...
				let emoji: string | GuildEmoji = interaction.options.getString('emoji')
				...
				let doc = await reactSchema.findOne<reactRole>({ guildId: guild.id, messageId: messageID })
				if (!doc) {
			...
			guild.channels.fetch(id).then(async (channel: TextChannel) => {
				...
				let role: string | Role = roles.get(emoji)
				roles.delete(emoji)
	...
	async run(message: Message, args: string[]) {
		...
		if (args[0] === 'add') {
			...
			guild.channels.fetch(channelStr).then(async (channel: TextChannel) => {
				...
				let emoji: string | GuildEmoji = args.shift()
				const reactEmoji = emoji
				if (emoji.startsWith('<a:') || emoji.startsWith('<:'))
					emoji = emoji.split(':')[2].replace('>', '')
				
				let toggle: string | boolean = args.shift()
