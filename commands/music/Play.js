...
module.exports = class extends Command {
    ...
    async executeSlash(interaction) {
        ...
        if (url.includes('youtube.com/playlist') || url.includes('open.spotify.com/playlist')) {
            await queue.playlist(url, { requestedBy: interaction.user }).catch((err) => {
                if (!queue) queue.stop()
            })
        } else {
            await queue.play(url, { requestedBy: interaction.user }).catch((err) => {
                if (!queue) queue.stop()
            })
        }
    }
    ...
    async run(message, args) {
        ...
        if (url.includes('youtube.com/playlist') || url.includes('open.spotify.com/playlist')) {
            await queue.playlist(url, { requestedBy: message.author }).catch((error) => {
                if (!queue) queue.stop()
            })
        } else {
            await queue.play(url, { requestedBy: message.author }).catch((error) => {
                if (!queue) queue.stop()
            })
        }
    }
