...
module.exports = class extends Command {
    ...
    async executeSlash(interaction) {
        ...
        axios.get(TenorAPI).then(response => {
            ...
            const embed = new EmbedBuilder()
                .setTitle(`${interaction.user.tag} gives cake to everyone`)
            ...
            if (user != null) embed.setTitle(`${interaction.user.tag} gives cake to ${user.tag}`)
    ...
    async run(message) {
        axios.get(TenorAPI).then(response => {
            ...
            const embed = new EmbedBuilder()
                .setTitle(`${message.author.tag} gives cake to everyone`)
                .setDescription(`[Link to image](${json.url})`)
                .setImage(json.url)
                .setColor(Colors.NotQuiteBlack)
                .setTimestamp()
            ...
            if (user != null) embed.setTitle(`${message.author.tag} gives cake to ${user.tag}`)
