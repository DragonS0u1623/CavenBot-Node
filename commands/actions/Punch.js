...
module.exports = class extends Command {
    constructor(client, name='punch') {
        super(client, name, {
            category: 'Action',
            description: 'Sends an embed with a gif of someone being punched',
            slash: true,
    ...
    async executeSlash(interaction) {
        ...
        axios.get(TenorAPI).then(response => {
            ...
            const embed = new EmbedBuilder()
                .setTitle(`${interaction.user.tag} punches everyone`)
            ...
            if (user != null) embed.setTitle(`${interaction.user.tag} punches ${user.tag}`)
    ...
    async execute(message) {
        axios.get(TenorAPI).then(response => {
            ...
            const embed = new EmbedBuilder()
                .setTitle(`${message.author.tag} punches everyone`)
                .setDescription(`[Link to image](${json.url})`)
                .setImage(json.url)
                .setColor(Colors.NotQuiteBlack)
                .setTimestamp(new Date())
            ...
            if (user != null) embed.setTitle(`${message.author.tag} punches ${user.tag}`)
