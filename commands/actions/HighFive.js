...
module.exports = class extends Command {
    constructor(client, name='highfive') {
        super(client, name, {
            aliases: ['hf'],
            category: 'Action',
            description: 'Sends an embed with a gif of someone being high fived',
            slash: true,
    ...
    async executeSlash(interaction) {
        ...
        axios.get(TenorAPI).then(response => {
            ...
            const embed = new EmbedBuilder()
                .setTitle(`${interaction.user.tag} highfives everyone`)
            ...
            if (user != null) embed.setTitle(`${interaction.user.tag} highfives ${user.tag}`)
    ...
    async execute(message) {
        axios.get(TenorAPI).then(response => {
            ...
            const embed = new EmbedBuilder()
                .setTitle(`${message.author.tag} highfives everyone`)
                .setDescription(`[Link to image](${json.url})`)
                .setImage(json.url)
                .setColor(Colors.NotQuiteBlack)
                .setTimestamp(new Date())
            ...
            if (user != null) embed.setTitle(`${message.author.tag} highfives ${user.tag}`)
